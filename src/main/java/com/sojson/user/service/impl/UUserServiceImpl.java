...
public class UUserServiceImpl extends BaseMybatisDao<UUserMapper> implements UUserService {
     ...
     */
    @Override
    public CommResp<UUser> addMemberUser(UUser record) {
        CommResp<UUser> commResp = new CommResp<>();
        commResp.setCode(BusinessCode.FAIL.getCode());
        commResp.setMessage(BusinessCode.FAIL.getMessage());

        //邮箱已被占用
        if (null != userMapper.findUserByEmail(record.getEmail())) {
            commResp.setMessage("邮箱已被占用");
            return commResp;
        } else {
            try {
                //保存用户，成功
                record.setPswd(UserManager.md5Pswd(record.getEmail(), record.getPswd()));
                record.setStatus(1l);
                record.setCreateTime(new Date());

                insert(record);
                commResp.setCode(BusinessCode.SUCCESS.getCode());
                return commResp;
            } catch (Exception e) {
                //保存用户失败
                return commResp;
