...
<head>
    ...
    <script>
         ...
         *根据角色ID选择权限，分配权限操作。
         */
        function selectRoleById(id, currRoleIds) {
            var load = layer.load();
            $.post("<%=basePath%>/role/selectRoleByUserId.shtml", {id: id}, function (result) {
                layer.close(load);
                var currRoleAry = currRoleIds.split(',')
                if (result && result.length) {
                    ...
                    $.each(result, function () {
                        ...
                        html.push(this.id);
                        html.push("'");
                        if (null != currRoleAry && currRoleAry.length > 0) {
                            for (var i = 0; i < currRoleAry.length; i++) {
                                if (this.id == parseInt(currRoleAry[i])) {
                                    ...
                                    <td>
                                        <shiro:hasPermission name="/role/addRole2User.shtml">
                                            <i class="glyphicon glyphicon-share-alt"></i><a
                                                href="javascript:selectRoleById('${it.id}','${it.roleIds}');">选择角色</a>
                                        </shiro:hasPermission>
                                    </td>
                                </tr>
                            </c:forEach>
