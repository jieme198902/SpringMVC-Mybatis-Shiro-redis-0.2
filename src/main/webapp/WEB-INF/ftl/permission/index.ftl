...
<head>
    ...
    <script>
        ...
        <#--修改权限-->
        <@shiro.hasPermission name="/permission/updatePermission.shtml">
         ...
         * @param url
         */
        function showUpdatePermissionDialog(id, name, url) {
            $('#uid').val(id)
            $('#uname').val(name)
            $('#uurl').val(url)
        ...
        <#--loding-->
            var load = layer.load();
            $.post('${basePath}/permission/updatePermission.shtml', {
                id: uidVal,
                name: nameVal,
                url: urlVal
            }, function (result) {
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <@shiro.hasPermission name="/permission/updatePermission.shtml">
                                    ...
                                    <a onclick="showUpdatePermissionDialog('${it.id}', '${it.name}', '${it.url}')">修改</a>
                                </@shiro.hasPermission>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="addPermissionLabel">修改权限</h4>
                </div>
                <div class="modal-body">
                    <form id="boxRoleForm">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="recipient-name" class="control-label">权限URL地址:</label>
                            <input type="text" class="form-control" id="uurl" name="url" placeholder="请输入权限URL地址">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <button type="button" onclick="updatePermissionItem();" class="btn btn-primary">Save</button>
                </div>
            </div>
        </div>
    </div>
