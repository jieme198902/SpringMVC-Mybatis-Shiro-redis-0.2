...
<head>
    ...
    <script>
        ...
        *根据角色ID选择权限，分配权限操作。
        */
        function selectPermissionById(id, permissionNames, permissionIds) {
            var userPermissionIds = permissionIds.split(",")
            var load = layer.load();
            $.post("${basePath}/permission/selectPermissionById.shtml", {id: id}, function (result) {
                layer.close(load);
                if (result && result.length) {
                    var html = [];
                    html.push('<div class="checkbox"><label><input type="checkbox"  selectAllBox=""  ');
                    if(userPermissionIds.length==result.length){
                        html.push(" checked='checked'");
                    }
                    html.push('>全选</label></div> ')
                    $.each(result, function () {
                        ...
                        for (var i = 0; i < userPermissionIds.length; i++) {
                            if (this.id == parseInt(userPermissionIds[i])) {
                                hasPermission = true
                                break;
                            }
                        }
                            ...
                            <td>
                                <@shiro.hasPermission name="/permission/addPermission2Role.shtml">
                                    <i class="glyphicon glyphicon-share-alt"></i><a
                                        href="javascript:selectPermissionById(${it.id},'${it.permissionNames}','${it.permissionIds}');">选择权限</a>
                                </@shiro.hasPermission>
                            </td>
                        </tr>
                    </#list>
